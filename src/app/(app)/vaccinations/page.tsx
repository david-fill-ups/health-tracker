...
export default function VaccinationsPage() {
  ...
  function handleDeleteVaccination(id: string) {
  ...
  const sortedVaccinations = useMemo(() => {
    return [...vaccinations].sort((a, b) => {
      ...
      const order = (STATUS_SORT_ORDER[aStatus] ?? 3) - (STATUS_SORT_ORDER[bStatus] ?? 3);
  ...
  const unrecordedVaccines = useMemo(() => {
    const recordedNames = new Set(vaccinations.map((v) => v.name.toLowerCase()));
  ...
  function getComplianceBadge(vaccinationName: string): ({ label: string; classes: string; subtitle: string | null }) | null {
    const rec = recByName.get(vaccinationName.toLowerCase());
          ...
          {sortedVaccinations.length === 0 ? (
            ...
            <div className="space-y-2">
              {sortedVaccinations.map((v) => {
                ...
                const badge = getComplianceBadge(v.name);
                return (
                  <div key={v.id} className="rounded-xl border border-gray-200 bg-white overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-3.5 hover:bg-gray-50">
                      <div className="flex flex-col min-w-0">
                        <span className="flex items-center gap-2">
                          <Link
                            href={`/vaccinations/${vaccineToSlug(v.name)}`}
                            className="font-semibold text-gray-900 hover:text-indigo-600"
                          >
                            {v.name}
                          </Link>
                      ...
                      >
                        {v.doses.length} {v.doses.length === 1 ? "dose" : "doses"}
                    ...
                    {isOpen && (
                      <div className="border-t border-gray-100 divide-y divide-gray-100">
                        {v.doses.length === 0 ? (
                          <div className="px-5 py-3 text-sm text-gray-400">No doses recorded.</div>
                        ) : (
                          v.doses.map((dose) => (
                            <div key={dose.id} className="flex items-center justify-between px-5 py-3 gap-4">
                              ...
                              <Link
                                href={`/vaccinations/${vaccineToSlug(v.name)}/doses/${dose.id}/edit`}
                        ...
                        <div className="px-5 py-2.5 flex justify-between items-center">
                          <Link
                            href={`/vaccinations/new?name=${encodeURIComponent(v.name)}`}
                          ...
                          <button
                            onClick={async () => {
                              if (!confirm(`Delete all records for "${v.name}"?`)) return;
                              const res = await fetch(`/api/vaccinations/${v.id}`, { method: "DELETE" });
                              if (res.ok) handleDeleteVaccination(v.id);
                            }}
